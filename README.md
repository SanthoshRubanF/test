...
# Knowledge Graph Builder for (Knowledge_Graph_ocr_poc)

This project is a powerful tool designed to automatically build and update a knowledge graph. It works by connecting to your ChromaDB Cloud instance, extracting document summaries, and then using the **Gemini 2.5 Flash** model to identify key entities and relationships. These relationships are then formatted into knowledge triples and seamlessly ingested into a **Neo4j AuraDB** graph database.

This tool is designed to be run as a standalone process to integrate newly ingested documents into your existing knowledge graph, enriching your data with a structured, interconnected representation.

-----

## ✨Features

- **Automated Extraction**: Fetches document summaries from a specified ChromaDB collection.
- **Gemini-Powered NLP**: Utilizes the high-speed and accurate Gemini 2.5 Flash model for robust entity and relationship extraction.
- **Knowledge Graph Creation**: Transforms extracted information into structured knowledge triples.
- **Neo4j Integration**: Inserts triples directly into Neo4j AuraDB, creating `Entity` nodes and `RELATION` relationships.
- **Streamlit UI**: Provides a simple, interactive user interface to run the pipeline and visualize the process.
- **Debug Mode**: A handy feature for developers to see the raw Gemini output and validated triples.

-----

## 🚀 Getting Started

Follow these steps to set up and run the Knowledge Graph Builder.

### 📋 Prerequisites

You'll need access to the following services and have their credentials ready:

- **Google AI Studio**: For a `GOOGLE_API_KEY` to access the Gemini API.
- **Neo4j AuraDB**: For `NEO4J_URI`, `NEO4J_USER`, and `NEO4J_PASSWORD`.
- **ChromaDB Cloud**: For `CHROMA_API_KEY`, `CHROMA_TENANT_ID`, and `CHROMA_DB_NAME`.
...
1. **Clone the repository:**
    ...
    ```bash
    git clone https://github.com/ConnITLabs/magi-poc-ocr-processing
    cd magi-poc-ocr-processing
    ```
...
2. **Install the required libraries:**
    ...
    *If you don't have a `requirements.txt` file, you can create one with the following packages:*
...
CHROMA_DB_NAME="your_chroma_db_name_here"
CHROMA_COLLECTION="your_chroma_collection_name_here"
```
...
1. **Run the Streamlit application:**
    ...
    ```bash
    streamlit run Knowledge_Graph_ocr_poc.py
    ```

2. **Open your browser:** A new browser window will open with the Streamlit application.

3. **Run the pipeline:**

      - Check the `🛠️ Enable Debug Mode` box in the sidebar if you want to see detailed output.
      - Click the **Run Pipeline** button. The application will automatically connect to ChromaDB, process the documents, and insert the extracted knowledge into Neo4j.

-----

## ⚙️How It Works

The core of this application is a pipeline that processes documents in three main stages:

1. **Data Ingestion**: The `get_summaries()` function connects to your ChromaDB Cloud instance and retrieves all documents from the specified collection.
2. **Information Extraction**: The `extract_entities_relationships_batch()` function sends each document summary to the Gemini 2.5 Flash model. It uses a carefully crafted prompt to ensure the model returns a predictable JSON format containing a list of entities and `[source, type, target]` relationships.
3. **Knowledge Graph Population**: The `insert_triples()` function takes the validated JSON output and uses the Neo4j Python driver to create or update the graph. It uses `MERGE` clauses to ensure that entities are created only if they don't already exist, and then creates the specified relationships between them.
